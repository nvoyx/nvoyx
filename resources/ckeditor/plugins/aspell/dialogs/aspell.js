...
{;
	...
	function spellTime(dialog, errorMsg)
	{
		...
		return function()
		{
			if (typeof window.spellChecker == 'function')
			{
				...
				/* NVOYX dynamic language */
				var nvx_lang = document.querySelectorAll("." + dialog.getParentEditor().id);
				nvx_lang = nvx_lang[0].id + "-language";
				nvx_lang = nvx_lang.replace("cke_","");
				nvx_lang = document.getElementById(nvx_lang).innerHTML;
				oSpeller.spellCheckScript = editor.plugins.aspell.path+'spellerpages/server-scripts/spellchecker.php?nvx_lang='+nvx_lang;
	...
	return {
		...
		onShow: function()
		{
			...
			/* NVOYX avoid loading of multiple instances of spellChecker.js */
			if(document.head.querySelectorAll("script[src='" + editor.plugins.aspell.path+'spellerpages/spellChecker.js' + "']").length === 0){
