...
foreach($field["fid-{$outline["fid"]}"] as $iteration=>$values){
	...
	<div class='col sml100 med50 lge33 pad-r10 sml-pad-r0 pad-b40'>
		...
		<input class='col all100 fs14 tb' name='<?="textbox-{$group["id"]}-{$vari}-{$outline["fid"]}-{$iteration}-text";?>' id='<?="textbox-{$group["id"]}-{$vari}-{$outline["fid"]}-{$iteration}-text";?>' type='text' maxlength='<?=$outline["maxlength"];?>' value='<?=htmlentities($v,ENT_QUOTES);?>'>
	</div>
	<?php
}